...
df_src = spark.sql('''
SELECT DISTINCT(Dealer_ID) AS Dealer_ID, DealerName FROM parquet.`abfss://silver@datalakeforcar.dfs.core.windows.net/carsales`
''')
...
if spark.catalog.tableExists('cars_catalog.gold.dim_dealer'):
    delta_tbl = DeltaTable.forPath(spark, "abfss://gold@datalakeforcar.dfs.core.windows.net/dim_dealer")
...
#Initial RUN
    ...
    df_final.write.format("delta")\
            .mode('overwrite')\
            .option("path","abfss://gold@datalakeforcar.dfs.core.windows.net/dim_dealer")\
