...
df_src = spark.sql('''
SELECT DISTINCT(Branch_ID) AS Branch_ID, BranchName FROM parquet.`abfss://silver@datalakeforcar.dfs.core.windows.net/carsales`
''')
...
if spark.catalog.tableExists('cars_catalog.gold.dim_branch'):
    delta_tbl = DeltaTable.forPath(spark, "abfss://gold@datalakeforcar.dfs.core.windows.net/dim_branch")
...
#Initial RUN
    ...
    df_final.write.format("delta")\
            .mode('overwrite')\
            .option("path","abfss://gold@datalakeforcar.dfs.core.windows.net/dim_branch")\
